...
This query looks across all commercial tax credit projects and returns the following: 
    ...
    - Missing Part 2 SHPO Financials (State Fees)
    - Missing Part 3 SHPO Financials (State Fees)
    - Missing All SHPO Financials (State Fees)
...
CASE
	WHEN Sub_Final.LatestDecisionPart IN ('Part 2', 'Part 3')  AND (
		tc.PartTwoStateFeeAmountPaid IS NULL
		)
	THEN 1
	ELSE 0
END AS MissingPart2_SHPO_Financials,
CASE	
	WHEN Sub_Final.LatestDecisionPart = 'Part 3' AND (
		tc.PartThreeStateFeeAmountPaid IS NULL)
	THEN 1
	ELSE 0
END AS MissingPart3_SHPO_Financials,
CASE	
	WHEN Sub_Final.LatestDecisionPart = 'Part 3' AND (
		tc.PartTwoStateFeeAmountPaid IS NULL AND 
		tc.PartThreeStateFeeAmountPaid IS NULL)
	THEN 1
	ELSE 0
END AS Missing_All_SHPO_Financials
FROM Sub_Final
...
SELECT * FROM Final
WHERE (MissingPart2_SHPO_Financials = 1
OR MissingPart3_SHPO_Financials = 1
OR Missing_All_SHPO_Financials = 1)
