...
CASE	
	WHEN Sub_Final.LatestDecisionPart = 'Part 3' AND (
		tc.PartThreeEstimatedRehabCost IS NULL)
	THEN 1
	ELSE 0
...
CASE
    WHEN Sub_Final.LatestDecisionPart <> 'Part 3' AND (
        tc.PartThreeNumHousingUnitsRehabBefore IS NOT NULL OR 
		tc.PartThreeNumHousingUnitsRehabAfter IS NOT NULL OR 
		tc.PartThreeNumLowModIncomeHousingUnitsRehabBefore IS NOT NULL OR 
		tc.PartThreeNumLowModIncomeHousingUnitsRehabAfter IS NOT NULL OR
        tc.PartThreeEstimatedRehabCost IS NOT NULL OR
        tc.PartThreeTotalEstimatedProjectCost IS NOT NULL
    )
    THEN 1
    ELSE 0
END AS MissingPart3_Decision
FROM Sub_Final
